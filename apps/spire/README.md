...
| `SPIRE_FCM_SERVICE_ACCOUNT_JSON`                                                                                            | _(unset)_    | Firebase service-account JSON used for Android native call wake pushes sent to `fcmCall` subscriptions. This lets closed Android apps receive high-priority data-only `callWake` payloads through FCM HTTP v1. Instead of the JSON blob, you may set `SPIRE_FCM_CLIENT_EMAIL`, `SPIRE_FCM_PROJECT_ID`, and either `SPIRE_FCM_PRIVATE_KEY` or `SPIRE_FCM_PRIVATE_KEY_FILE`.                             |
| `SPIRE_APNS_TEAM_ID`, `SPIRE_APNS_KEY_ID`, `SPIRE_APNS_BUNDLE_ID`, `SPIRE_APNS_PRIVATE_KEY` / `SPIRE_APNS_PRIVATE_KEY_FILE` | _(unset)_    | Apple APNs provider credentials used for iOS native VoIP call wake pushes sent to `apnsVoip` subscriptions. `SPIRE_APNS_TOPIC` defaults to `<bundle id>.voip`; set it only if Apple expects a different VoIP topic.                                                                                                                                                                                    |
| `SPIRE_APNS_ENV`                                                                                                            | `production` | APNs endpoint for native VoIP call wake pushes. Set to `sandbox` for development builds signed against the APNs sandbox environment.                                                                                                                                                                                                                                                                   |
